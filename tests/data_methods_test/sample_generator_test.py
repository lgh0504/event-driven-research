...
def creator_test(twitter_database_path, stock_database_path, lookup_table_path):
    start_date = '2018-04-23'
    start_index = 0
    end_date = '2018-04-27'
    end_index = 390
    sample_generator = SampleGenerator(twitter_database_path, stock_database_path, lookup_table_path,
                                       start_date, start_index, end_date, end_index)
    twittwe_sample = sample_generator.get_serial_text_sample('aapl', 1954)
...
if __name__ == "__main__":
    ...
    lookup_path = path.join(root_path, "resources/lookup_table.txt")

    # test
    creator_test(twitter_db_path, stock_db_path, lookup_path)
